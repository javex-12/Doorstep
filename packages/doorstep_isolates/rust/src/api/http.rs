...
pub use doorstep_core::http::client::{ClientError, LsHttpClientVersion};
pub use doorstep_core::http::dto::{
    PrepareUploadRequestDto, PrepareUploadResponseDto, PrepareUploadResult, ProtocolType,
    RegisterDto, RegisterResponseDto,
};
use doorstep_core::reqwest;

pub struct RsHttpClient {
    inner: doorstep_core::http::client::LsHttpClient,
}
...
pub fn create_client(
    ...
    let inner = doorstep_core::http::client::LsHttpClient::new(
        &private_key,
        &cert,
        version,
...
fn resolve_file_content(
    ...
    path: Option<String>,
    file_descriptor: Option<i32>,
) -> Result<doorstep_core::model::transfer::FileContent, RsHttpClientError> {
    match (binary, path, file_descriptor) {
        (Some(binary), None, None) => Ok(doorstep_core::model::transfer::FileContent::Stream(
            binary.receiver,
        )),
        (None, Some(path), None) => Ok(doorstep_core::model::transfer::FileContent::Path(path.into())),
        (None, None, Some(file_descriptor)) => {
            #[cfg(target_os = "android")]
            {
                Ok(doorstep_core::model::transfer::FileContent::Fd(file_descriptor))
            }
