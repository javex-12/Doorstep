...
pub fn verify_cert(cert: String, public_key: String) -> anyhow::Result<()> {
    doorstep_core::crypto::cert::verify_cert_from_pem(cert, Some(&public_key))
}

pub fn generate_key_pair() -> anyhow::Result<KeyPair> {
    let signing_key = doorstep_core::crypto::token::generate_key();
    let private_key = doorstep_core::crypto::token::export_private_key(&signing_key)?;
    let public_key = doorstep_core::crypto::token::export_public_key(&signing_key)?;
...
pub async fn hash_file(
    ...
    let content = match (path, file_descriptor, bytes) {
        (Some(path), None, None) => doorstep_core::model::transfer::FileContent::Path(path.into()),
        (None, Some(file_descriptor), None) => {
            #[cfg(target_os = "android")]
            {
                doorstep_core::model::transfer::FileContent::Fd(file_descriptor)
            }
        ...
        (None, None, Some(bytes)) => return Ok(doorstep_core::crypto::hash::sha256_hex(&bytes)),
        _ => anyhow::bail!("Exactly one content source must be provided"),
    };

    Ok(doorstep_core::crypto::hash::sha256_file_content(content, &cancel_token.inner).await?)
}
