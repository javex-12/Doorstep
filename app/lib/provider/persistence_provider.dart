...
String get _windowsFile {
  ...
  return '$appData\\Doorstep\\settings.json';
}

String get _windowsLegacyFile {
  final appData = Platform.environment['APPDATA'];
  return '$appData\\doorstep\\shared_preferences.json';
}
...
// Doorstep
const _pairedDevices = 'doorstep_paired_devices';
const _watchedFolders = 'doorstep_watched_folders';
...
class PersistenceService {
  ...
  static Future<void> _initColorSetting(SharedPreferences prefs, bool supportsDynamicColors) async {
    await prefs.setString(
      _colorKey,
      checkPlatform([TargetPlatform.android]) && supportsDynamicColors ? ColorMode.system.name : ColorMode.doorstep.name,
    );
  }
  ...
  List<String> getPairedDevicesRaw() {
    return _prefs.getStringList(_pairedDevices) ?? [];
  }

  Future<void> setPairedDevicesRaw(List<String> entries) async {
    await _prefs.setStringList(_pairedDevices, entries);
  }

  List<String> getWatchedFoldersRaw() {
    return _prefs.getStringList(_watchedFolders) ?? [];
  }

  Future<void> setWatchedFoldersRaw(List<String> entries) async {
    await _prefs.setStringList(_watchedFolders, entries);
  }

  /// The persistent Doorstep pairing token of *this* device.
  ///
  /// The laptop embeds it in its pairing QR; the phone sends it back during
  /// the pairing handshake so the laptop can verify the scan. Both devices
  /// keep their own token — it is the long-lived trust anchor of the pair.
  String? getDoorstepOwnToken() {
    return _prefs.getString(_doorstepOwnToken);
  }

  Future<void> setDoorstepOwnToken(String token) async {
    await _prefs.setString(_doorstepOwnToken, token);
  }

  /// Whether transfers from paired devices are accepted without prompting
  /// (default true). When disabled, paired devices go through the normal
  /// accept flow.
  bool getDoorstepAutoAccept() {
    return _prefs.getBool(_doorstepAutoAccept) ?? true;
  }

  Future<void> setDoorstepAutoAccept(bool value) async {
    await _prefs.setBool(_doorstepAutoAccept, value);
  }

  /// Battery-saving sleep mode (phone side): pauses reconnect announcements
  /// while active, so auto-transfer does not work until it is turned off.
  bool getDoorstepSleepMode() {
    return _prefs.getBool(_doorstepSleepMode) ?? false;
  }
