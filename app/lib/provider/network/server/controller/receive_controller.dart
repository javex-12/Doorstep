...
import 'package:doorstep_app/provider/selection/selected_receiving_files_provider.dart';
...
import 'package:doorstep_app/widget/dialogs/open_file_dialog.dart';
...
import 'package:doorstep_isolates/rust/api/server.dart' show RegisterDtoV2, SessionEndReasonV2;
...
class ReceiveController {
  ...
  Future<void> onRegister(HttpServerRegisterEvent event) async {
    ...
    final cleanModel = DoorstepPairingHandshake.parse(event.info.deviceModel)?.cleanDeviceModel;
    final info = cleanModel == null
        ? event.info
        : RegisterDtoV2(
            alias: event.info.alias,
            version: event.info.version,
            deviceModel: cleanModel,
            deviceType: event.info.deviceType,
            fingerprint: event.info.fingerprint,
            port: event.info.port,
            protocol: event.info.protocol,
            download: event.info.download,
          );

    await server.ref.redux(nearbyDevicesProvider).dispatchAsync(RegisterDeviceAction(info.toDevice(event.ip, HttpDiscovery(ip: event.ip))));
  ...
  Future<void> onPrepareUpload(HttpServerPrepareUploadEvent event) async {
    ...
    // Doorstep: transfers from a paired device are accepted automatically,
    // without any prompt or progress page — the file just arrives. It shows up
    // in the received-files view with the arrival animation. Can be disabled
    // in Settings (then paired devices use the normal accept flow).
    final doorstep = server.ref.read(doorstepSettingsProvider);
    final isDoorstepPaired = server.ref.read(doorstepPairingProvider).any((d) => d.fingerprint == senderFingerprint);
    // Sleep mode pauses the silent Doorstep pipeline: transfers still arrive,
    // but go through the normal accept flow instead of being auto-accepted.
    if (isDoorstepPaired && doorstep.autoAcceptFromPaired && !doorstep.sleepMode) {
      // A text message from a paired device must still be recorded in history
      // (this branch normally returns before the message-recording block below).
      await _recordMessageIfPresent();
  ...
  Future<void> onFileUploadResult(HttpServerFileUploadResultEvent event) async {
    ...
    if (allowedStates.contains(session.status) && session.files.values.map((e) => e.status).isFinishedOrError) {
      ...
      // Doorstep transfers are fully silent: no dialog, no page push — the
      // arrival animation in the Activity tab is the whole notification.
      final doorstep = server.ref.read(doorstepSettingsProvider);
      final isDoorstepPaired = server.ref.read(doorstepPairingProvider).any((d) => d.fingerprint == session.sender.fingerprint);
      if (isDoorstepPaired && doorstep.autoAcceptFromPaired && !doorstep.sleepMode && !hasError) {
        Future.delayed(Duration.zero, () {
          closeSession();
          _logger.info('Closing Doorstep session');
        });
      } else if (quickSave) {
  ...
  /// Records a text message from the current receive session into history.
  Future<void> _recordMessageIfPresent() async {
    final message = server.getState().session?.message;
    if (message == null) {
      return;
    }
