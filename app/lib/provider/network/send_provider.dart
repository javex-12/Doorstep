...
import 'package:doorstep_app/pages/send_page.dart';
...
import 'package:doorstep_isolates/model/dto/file_dto.dart';
...
import 'package:doorstep_isolates/rust/api/cancel.dart' as rust_cancel;
import 'package:doorstep_isolates/rust/api/http.dart' as rust_http;
import 'package:doorstep_isolates/rust/api/model.dart' as rust_model;
import 'package:doorstep_isolates/util/file_hash.dart';
...
class SendNotifier extends Notifier<Map<String, SendSessionState>> {
  ...
  /// Returns the session id, which is null when the session was canceled before it started.
  Future<String?> startSession({
    ...
    if (fileMap.isEmpty) {
      ...
      if (state[sessionId]?.background == false) {
        // ignore: use_build_context_synchronously, unawaited_futures
        Routerino.context.pushRootImmediately(() => const HomePage(initialTab: HomeTab.doorstep, appStart: false));
      }
