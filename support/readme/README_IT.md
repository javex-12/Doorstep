...
Doorstep è un'applicazione gratuita e open-source che consente di condividere file e messaggi, in modo sicuro, con i dispositivi vicini attraverso la rete locale, senza bisogno di una connessione a Internet.
...
Doorstep è un'applicazione multipiattaforma che permette di comunicare in modo sicuro tra dispositivi utilizzando un'API REST e la crittografia HTTPS. A differenza di altre app di messaggistica che si affidano a server esterni, Doorstep non richiede una connessione a Internet o a server di terze parti e rappresenta una soluzione veloce e affidabile per la comunicazione locale.
...
Nella maggior parte dei casi, Doorstep dovrebbe funzionare immediatamente. Tuttavia, se si riscontrano problemi nell'invio o nella ricezione di file, potrebbe essere necessario configurare il firewall per consentire a Doorstep di comunicare su rete locale.
...
Per avviare l'app in modalità minimizzata (solo nella barra delle applicazioni), usa il flag `--hidden` (esempio: `doorstep.exe --hidden`).
...
Doorstep utilizza un protocollo di comunicazione sicura che premette ai dispositivi di comunicare a vicenda tramite una API REST. Tutti i dati vengono inviati in modo sicuro tramite HTTPS e il certificato TLS/SSL è generato sul momento su ogni dispositivo, assicurando la massima sicurezza.

Per maggiori informazioni sul Protocollo Doorstep, consulta la [documentazione](https://github.com/localsend/protocol).

## Primi passi

Segui questi passi per compilare Doorstep dal codice sorgente:
...
3. Clona la repository di `Doorstep`
...
> [!NOTE]
> Attualmente Doorstep richiede una versione più datata di Flutter (specificata in [.fvmrc](.fvmrc))
...
> Per rendere lo sviluppo più consistente, Doorstep utilizza [fvm](https://fvm.app) per gestire la versione di Flutter del progetto.
> Dopo aver installato `fvm`, esegui `fvm flutter` anziché `flutter`.

## Come contribuire

Accogliamo con piacere i contributi di chiunque sia interessato a migliorare Doorstep. Se si desidera contribuire ci sono diversi modi per partecipare:

### Traduzione

È possibile contribuire traducendo Doorstep in altre lingue. **Metodo consigliato**: Utilizza la piattaforma [Weblate](https://hosted.weblate.org/projects/localsend/app) per gestire le traduzioni.
...
- **Miglioramenti:** Hai un'idea su come poter migliorare Doorstep? Prima di tutto crea una "issue" per discutere il motivi per cui il miglioramento è necessario.
