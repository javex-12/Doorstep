...
Doorstep, internet bağlantısına ihtiyaç duymadan, yerel ağ üzerinden yakın cihazlarla güvenli bir şekilde dosya ve mesaj paylaşmanıza olanak tanıyan ücretsiz, açık kaynaklı bir uygulamadır.
...
Doorstep, cihazlar arasında güvenli iletişim sağlamak için REST API ve HTTPS şifrelemesi kullanan, çoklu platform desteğine sahip bir uygulamadır. Diğer mesajlaşma uygulamalarının dış sunuculara bağımlı olmasının aksine, Doorstep internet bağlantısına veya üçüncü taraf sunuculara ihtiyaç duymaz; bu da yerel iletişim için hızlı ve güvenilir bir çözüm sunar.
...
Çoğu durumda, Doorstep kutudan çıkar çıkmaz çalışmalıdır. Ancak, dosya gönderme veya alma konusunda sorun yaşıyorsanız, Doorstep'in yerel ağ üzerinden iletişim kurabilmesi için güvenlik duvarınızı yapılandırmanız gerekebilir.
...
Uygulamayı gizli başlatmak için (yalnızca sistem tepsisinde), `--hidden` bayrağını kullanın (örnek: `doorstep.exe --hidden`).

v1.14.0 ve önceki sürümlerde, `autostart` bayrağı ayarlandığında ve gizli ayarı etkinleştirildiğinde uygulama gizli başlar.

## Nasıl Çalışır

Doorstep, cihazların birbirleriyle iletişim kurmasını sağlayan güvenli bir iletişim protokolü kullanır ve bu iletişim REST API aracılığıyla gerçekleştirilir. Tüm veriler HTTPS üzerinden güvenli bir şekilde iletilir ve her cihazda anlık olarak oluşturulan TLS/SSL sertifikası ile maksimum güvenlik sağlanır.

Doorstep Protokolü hakkında daha fazla bilgi için [dökümantasyon](https://github.com/localsend/protocol).

## Başlarken

Doorstep'i kaynak kodundan derlemek için şu adımları izleyin:
...
> [!NOT]
> Doorstep, şu anda daha eski bir Flutter sürümünü gerektiriyor(gerekli sürüm [.fvmrc](.fvmrc) dosyasında belirtilmiştir)
...
> Geliştirmeyi daha tutarlı hale getirmek için Doorstep, proje Flutter sürümünü yönetmek için [fvm](https://fvm.app) kullanır.
> `fvm`'yi kurduktan sonra, `flutter` yerine `fvm flutter` komutunu çalıştırın.

## Katkıda Bulunma

Doorstep'i geliştirmekle ilgilenen herkesten katkı bekliyoruz. Katkıda bulunmak isterseniz, dahil olmanın birkaç yolu vardır:

### Çeviri

Doorstep'i diğer dillere çevirmeye yardımcı olabilirsiniz! **Önerilen yöntem**: Çevirileri yönetmek için [Weblate](https://hosted.weblate.org/projects/localsend/app) platformunu kullanın.
...
- **İyileştirmeler:** Doorstep'i geliştirmek için bir fikriniz var mı? Lütfen önce neden bu iyileştirmenin gerekli olduğunu tartışmak için bir issue oluşturun.
