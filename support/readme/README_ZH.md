...
Doorstep 是一个跨平台应用程序，使用 REST API 和 HTTPS 加密实现设备之间的安全通信。与依赖外部服务器的其他消息应用程序不同，Doorstep 不需要互联网连接或第三方服务器，因此成为本地通信的快速可靠解决方案。
...
在大多数情况下，Doorstep 应该可以直接使用。但是，如果你在发送或接收文件时遇到问题，可能需要配置防火墙以允许 Doorstep 在你的本地网络上通信。
...
Doorstep 使用安全通信协议，允许设备通过 REST API 进行通信。所有数据都通过 HTTPS 安全地发送，并且 TLS/SSL 证书会在每台设备上动态生成，确保最大的安全性。

欲了解更多关于 Doorstep 协议的信息，请参阅[文档](https://github.com/localsend/protocol)。

## 开始使用

要从源代码编译 Doorstep，请按照以下步骤进行操作：
...
> [!NOTE]
> Doorstep 目前需要老版本的 Flutter （具体见 [.fvmrc](/.fvmrc)）
> 因而一些构建问题也许是系统安装的 Flutter 版本和 Doorstep 所需的 Flutter 版本不一致导致的。
> 为了在开发过程中保持一致性，Doorstep 使用 [fvm](https://fvm.app) 来管理此项目的 Flutter 版本。
> 安装 `fvm` 后，请运行 `fvm flutter` 而非 `flutter` 。

## 贡献

我们欢迎任何有兴趣帮助改进 Doorstep 的人士贡献。如果你想贡献，有几种方式可以参与：
...
- **改进：** 有改进 Doorstep 的想法吗？请先创建一个问题来讨论为什么需要这个改进。
