...
Doorstep یک برنامه رایگان و متن‌باز است که به شما امکان می‌دهد تا فایل‌ها و پیام‌ها را با دستگاه‌های نزدیک از طریق شبکه محلی خود، بدون نیاز به اینترنت، به‌صورت امن به اشتراک بگذارید.
...
در بیشتر موارد، Doorstep باید به‌طور خودکار کار کند. با این حال، اگر در ارسال یا دریافت فایل‌ها مشکلی داشتید، ممکن است نیاز باشد تنظیمات فایروال خود را برای اجازه دسترسی Doorstep به شبکه محلی تغییر دهید.
...
برای شروع برنامه به‌صورت مخفی (فقط در قسمت اعلان)، از فلگ `--hidden` استفاده کنید (مثال: `doorstep.exe --hidden`).
...
برای اطلاعات بیشتر درباره پروتکل Doorstep، به مستندات مراجعه کنید. [مستندات](https://github.com/localsend/protocol).

## شروع به کار

برای کامپایل کردن Doorstep از سورس کد، مراحل زیر را دنبال کنید:
...
> [!NOTE]
> Doorstep لوکال‌سند در حال حاضر به نسخه‌ای قدیمی‌تر از فلاتر نیاز دارد ([مشخص شده در](.fvmrc) fvmrc) و بنابراین ممکن است مشکلاتی
> در بیلد به دلیل عدم هماهنگی بین نسخه مورد نیاز و نسخه نصب‌شده (در سطح سیستم) فلاتر رخ دهد. برای یکپارچه‌سازی توسعه،
> Doorstep از [fvm](https://fvm.app) برای مدیریت نسخه فلاتر پروژه استفاده می‌کند. پس از نصب `fvm،` به جای `flutter` از `fvm flutter` استفاده کنید.

## مشارکت

ما از مشارکت هر کسی که مایل به بهبود Doorstep است استقبال می‌کنیم. اگر مایل به کمک هستید، راه‌های مختلفی برای مشارکت وجود دارد:

### ترجمه

می‌توانید کمک کنید تا Doorstep را به زبان‌های دیگر ترجمه کنیم! **راه‌ی ترجیح‌داده شده**: از پلتفرم [Weblate](https://hosted.weblate.org/projects/localsend/app) برای مدیریت ترجمه‌ها استفاده کنید.
...
- **بهبودها:** ایده‌ای برای بهبود Doorstep دارید؟ لطفاً ابتدا یک issue ایجاد کنید تا درباره نیاز به این بهبود صحبت کنیم.
