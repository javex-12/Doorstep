...
Doorstep — це кросплатформна програма, яка забезпечує безпечний зв'язок між пристроями за допомогою REST API та HTTPS-шифрування. На відміну від інших програм для обміну повідомленнями, які покладаються на зовнішні сервери, Doorstep не потребує підключення до Інтернету чи сторонніх серверів, що робить його швидким та надійним рішенням для локального зв'язку.
...
У більшості випадків Doorstep має працювати прямо з коробки. Проте, якщо у вас виникли проблеми з надсиланням або отриманням файлів, можливо, вам доведеться налаштувати брандмауер, щоб дозволити Doorstep взаємодіяти з вашою локальною мережею.
...
Doorstep використовує безпечний протокол зв'язку, який дозволяє пристроям взаємодіяти між собою за допомогою REST API. Усі дані безпечно надсилаються через HTTPS протокол, а сертифікат TLS/SSL генерується на льоту на кожному пристрої, забезпечуючи максимальну безпеку.

Для отримання додаткової інформації про протокол Doorstep перегляньте [документацію](https://github.com/localsend/protocol).

## Початок роботи

Щоб скомпілювати Doorstep з вихідного коду, виконайте наступні кроки:
...
3. Клонуйте `Doorstep` репозиторій
...
> [!NOTE]
> Наразі Doorstep потребує старішої версії Flutter (вказаної в [.fvmrc](.fvmrc)),
...
> Щоб зробити розробку більш узгодженою, Doorstep використовує [fvm](https://fvm.app) для керування версією проекту Flutter.
> Після встановлення `fvm`, запустіть `fvm flutter` замість `flutter`.

## Внесок у розвиток проекту

Ми вітаємо внески від усіх, хто хоче допомогти покращити Doorstep. Якщо ви хочете зробити внесок, є кілька способів долучитися:

### Переклад

Ви можете допомогти перекласти Doorstep на інші мови. Ми використовуємо платформу [Weblate](https://hosted.weblate.org/projects/localsend/app) для керування перекладами.
...
- **Покращення:** Маєте ідею, як покращити Doorstep? Будь ласка, спочатку створіть відповідну тему, щоб обговорити, чому це покращення необхідне.
