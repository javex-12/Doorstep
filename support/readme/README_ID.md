...
Doorstep adalah aplikasi lintas platform yang memungkinkan komunikasi aman antar perangkat menggunakan REST API dan enkripsi HTTPS. Berbeda dengan aplikasi pesan lainnya yang mengandalkan server eksternal, Doorstep tidak memerlukan koneksi internet atau server pihak ketiga, menjadikannya solusi yang cepat dan andal untuk komunikasi lokal.
...
Dalam kebanyakan kasus, Doorstep seharusnya berfungsi langsung tanpa konfigurasi. Namun, jika Anda mengalami masalah dalam mengirim atau menerima file, Anda mungkin perlu mengonfigurasi firewall Anda untuk mengizinkan Doorstep berkomunikasi melalui jaringan lokal Anda.
...
Doorstep menggunakan protokol komunikasi yang aman, memungkinkan perangkat untuk berkomunikasi satu sama lain menggunakan REST API. Semua data dikirim dengan aman melalui HTTPS, dan sertifikat TLS/SSL dihasilkan secara langsung di setiap perangkat, memastikan keamanan maksimum.

Untuk informasi lebih lanjut tentang Protokol Doorstep, lihat [dokumentasi](https://github.com/localsend/protocol).

## Memulai

Untuk mengompilasi Doorstep dari kode sumber, ikuti langkah-langkah berikut:

1. Instal Flutter [langsung](https://flutter.dev) atau menggunakan [fvm](https://fvm.app) (lihat [versi yang diperlukan](.fvmrc))
2. Clone repositori `Doorstep`
...
> [!CATATAN]  
> Saat ini Doorstep memerlukan versi Flutter yang lebih lama (disebutkan dalam [.fvmrc](.fvmrc)) dan masalah build dapat terjadi karena ketidakcocokan antara versi Flutter yang diperlukan dan yang terinstal di sistem.  
> Untuk membuat pengembangan lebih konsisten, Doorstep menggunakan [fvm](https://fvm.app) untuk mengelola versi Flutter proyek.  
> Setelah menginstal `fvm`, jalankan `fvm flutter` alih-alih `flutter`.

## Berkontribusi

Kami menerima kontribusi dari siapa pun yang tertarik untuk membantu meningkatkan Doorstep. Jika Anda ingin berkontribusi, ada beberapa cara untuk terlibat:

### Terjemahan

Anda dapat membantu menerjemahkan Doorstep ke bahasa lain! **Cara yang direkomendasikan**: Gunakan platform [Weblate](https://hosted.weblate.org/projects/localsend/app) untuk mengelola terjemahan.
...
- **Peningkatan:** Punya ide untuk meningkatkan Doorstep? Silakan buat isu terlebih dahulu untuk mendiskusikan mengapa peningkatan tersebut diperlukan.
