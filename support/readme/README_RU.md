...
Doorstep — это бесплатное приложение с открытым исходным кодом, которое позволяет вам безопасно обмениваться файлами и сообщениями с находящимися поблизости устройствами через локальную сеть, без необходимости подключения к Интернету.

- [О Doorstep](#о-localsend)
- [Скриншоты](#скриншоты)
- [Скачать](#скачать)
...
Doorstep — это кроссплатформенное приложение, которое обеспечивает безопасную связь между устройствами с помощью REST API и HTTPS-шифрования. В отличие от других приложений для обмена сообщениями, которые полагаются на внешние серверы, Doorstep не требует подключения к Интернету или сторонним серверам, что делает его быстрым и надежным решением для локальной связи.
...
В большинстве случаев Doorstep должен работать прямо из коробки. Однако, если у вас возникли проблемы с отправкой или получением файлов, возможно, вам придется настроить брандмауэр, чтобы позволить Doorstep взаимодействовать с вашей локальной сетью.
...
Чтобы запустить приложение скрытно (только в трее), используйте флаг `--hidden` (например: `doorstep.exe --hidden`).
...
Doorstep использует безопасный протокол связи, который позволяет устройствам взаимодействовать между собой с помощью REST API. Все данные безопасно передаются через HTTPS протокол, а сертификат TLS/SSL генерируется на лету на каждом устройстве, обеспечивая максимальную безопасность.

Для получения дополнительной информации о протоколе Doorstep смотрите [документацию](https://github.com/localsend/protocol).

## Начало работы

Чтобы скомпилировать Doorstep из исходного кода, выполните следующие действия:
...
3. Клонируйте `Doorstep` репозиторий
...
> [!NOTE]
> На данный момент Doorstep требует более старой версии Flutter (указанной в [.fvmrc](.fvmrc)),
...
> Чтобы сделать разработку более согласованной, Doorstep использует [fvm](https://fvm.app) для управления версией проекта Flutter.
> После установки `fvm`, запустите `fvm flutter` вместо `flutter`.

## Вклад в развитие проекта

Мы приветствуем вклады от всех, кто хочет помочь улучшить Doorstep. Если вы хотите внести вклад, есть несколько способов принять участие:

### Перевод

Вы можете помочь перевести Doorstep на другие языки. **Рекомендуемый способ**: Используйте платформу [Weblate](https://hosted.weblate.org/projects/localsend/app) для управления переводами.
...
- **Улучшение:** Есть идея, как улучшить Doorstep? Пожалуйста, сначала создайте соответствующую тему, чтобы обсудить, почему это улучшение необходимо.
