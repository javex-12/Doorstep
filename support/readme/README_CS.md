...
Doorstep je bezplatná aplikace s otevřeným zdrojovým kódem, která vám umožňuje bezpečně sdílet soubory a zprávy s okolními zařízeními přes vaši místní síť, aniž byste potřebovali připojení k internetu.
...
Doorstep je multiplatformní aplikace, která umožňuje bezpečnou komunikaci mezi zařízeními pomocí REST API a šifrování HTTPS. Na rozdíl od jiných aplikací pro zasílání zpráv, které se spoléhají na externí servery, Doorstep nevyžaduje připojení k internetu ani servery třetích stran, což z něj činí rychlé a spolehlivé řešení pro místní komunikaci.
...
Ve většině případů by měl Doorstep fungovat hned. Pokud však máte potíže s odesíláním nebo přijímáním souborů, možná budete muset nakonfigurovat bránu firewall tak, aby umožnila Doorstep komunikovat přes vaši místní síť.
...
Chcete-li aplikaci spustit skrytě (pouze v liště), použijte příznak `--hidden` (příklad: `doorstep.exe --hidden`).
...
Doorstep používá zabezpečený komunikační protokol, který umožňuje zařízením komunikovat mezi sebou pomocí REST API. Všechna data jsou odesílána bezpečně přes HTTPS a TLS/SSL certifikát je generován za chodu na každém zařízení, což zajišťuje maximální bezpečnost.

Další informace o protokolu Doorstep najdete v [dokumentaci](https://github.com/localsend/protocol).

## Začínáme

Chcete-li zkompilovat Doorstep ze zdrojového kódu, postupujte následovně:
...
> Aby byl vývoj konzistentnější, používá Doorstep [fvm](https://fvm.app) ke správě verze projektu Flutter.
> Po instalaci `fvm` spusťte `fvm flutter` místo `flutter`.

## Jak přispět

Vítáme příspěvky od každého, kdo má zájem pomoci zlepšit Doorstep. Pokud chcete přispět, existuje několik způsobů, jak se zapojit:

### Překlady

Můžete pomoci s překladem Doorstep do jiných jazyků. Ke správě překladů používáme platformu [Weblate](https://hosted.weblate.org/projects/localsend/app).
...
- **Vylepšení:** Máte nápad, jak zlepšit Doorstep? Nejprve prosím vytvořte požadavek, abyste mohli prodiskutovat, proč je vylepšení potřeba.
