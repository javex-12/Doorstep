...
Doorstep jest darmową, otwartoźródłową aplikacją, która umożliwia Ci bezpiecznie udostępnianie plików oraz wiadomości pobliskim urządzeniom w Twojej sieci lokalnej bez konieczności połączenia z internetem.
...
Doorstep jest wieloplatformową aplikacją, która umożliwia bezpieczną komunikację między urządzeniami przy użyciu REST API oraz szyfrowania HTTPS. W przeciwieństwie do innych aplikacji, które bazują na zewnętrznych serwerach, Doorstep nie wymaga połączenia z internetem ani serwerów osób trzecich, co czyni go szybkim i niezawodnym rozwiązaniem dla lokalnej komunikacji.
...
[sklep play]: https://github.com/javex-12/Doorstep/releases/latest
...
W większości przypadków Doorstep powinien działać od razu po instalacji. Jednakże, jeśli masz problemy z wysyłaniem lub odbieraniem plików, może być konieczne skonfigurowanie Twojej zapory sieciowej, aby umożliwić Doorstep komunikację przez sieć lokalną.
...
Aby uruchomić aplikację do zasobnika, użyj flagi `--hidden` (przykład: `doorstep.exe --hidden`).
...
> [!NOTE]
> Doorstep obecnie wymaga starszej wersji Fluttera (określonej w [.fvmrc](.fvmrc))
...
> Aby uczynić rozwój bardziej spójnym, Doorstep używa [fvm](https://fvm.app) do zarządzania wersją projektu Flutter.
> Po zainstalowaniu `fvm`, wykonaj `fvm flutter` zamiast `flutter`.

## Wnoszenie wkładu

Zapraszamy wszystkich tych, którzy chcą pomóc w ulepszaniu Doorstep. Jeśli chcesz wnieść swój wkład, możesz zaangażować się na kilka sposobów:

### Tłumaczenie

Możesz pomóc tłumaczyć Doorstep na inne języki! **Rekomendowana metoda**: Używaj platformy [Weblate](https://hosted.weblate.org/projects/localsend/app) do zarządzania tłumaczeniami.
...
- **Ulepszenia:** Masz pomysł, jak ulepszyć Doorstep? Najpierw utwórz problem, aby przedyskutować, dlaczego ulepszenie jest potrzebne.
